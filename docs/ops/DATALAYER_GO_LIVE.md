...
- **`confirm.py` only does a local read-back** after a write — it does not poll
  `get_root().confirmed`. A transaction stalled in the mempool would advance the
  watermark while nothing reached a block. This is why the fee is set to
  0.0001 XCH rather than 0. Worth fixing before unattended scheduling.
